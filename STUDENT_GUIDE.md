...
dbt와 각종 스크립트를 실행하기 위한 독립적인 Python 환경을 만듭니다. **이후 모든 실습 과정은 가상 환경이 활성화된 상태에서 진행해야 합니다.**

```bash
# 1. 가상 환경 생성
python3.11 -m venv venv_dbt

# 2. 가상 환경 활성화 (실습 세션 내내 유지하세요)
...
## 📊 3단계: 데이터베이스 초기화 및 데이터 로드

실습에 사용할 스키마를 생성하고 원본 데이터를 DB에 적재합니다.

```bash
# 한 번에 모든 환경을 설정하는 스크립트를 실행합니다.
# (스키마 생성 -> DDL 실행 -> CSV 로드 -> 로그 테이블 생성)
. set_environments.sh
```
...
**💡 문제 발생 시**: 처음부터 다시 시작하고 싶다면 `1.1단계`부터 다시 실행하면 깨끗한 상태로 복구됩니다.
