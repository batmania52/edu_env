...
| 탭 | 역할 |
|----|------|
| 🚀 dbt Runner | 모델 실행, Run 범위 설정, Lineage 시각화 |
...
```
모델 선택 → (Run 범위 설정) → (Lineage 시각화) → 날짜 선택 → Compile → Command Review → Run → 검증 실행
```
...
### Run 범위 설정 (모델 선택 영역 하단)

Lineage 시각화와 독립적으로 dbt run의 `--select` / `--exclude` 범위를 설정.

| 컨트롤 | 설명 |
|--------|------|
| ⬆️ Upstream 체크박스 | 체크 시 선택 모델의 upstream을 run 범위에 포함 |
| Upstream Depth 숫자 입력 | upstream 포함 깊이 (1~10, 기본값 1) |
| ⬇️ Downstream 체크박스 | 체크 시 선택 모델의 downstream을 run 범위에 포함 |
| Downstream Depth 숫자 입력 | downstream 포함 깊이 (1~10, 기본값 1) |
| 🚫 Exclude 모델 멀티셀렉트 | run 범위에서 제외할 모델 선택 |

**Exclude 멀티셀렉트 동작:**
- upstream 또는 downstream 중 하나라도 체크하면 표시
- upstream 체크 시 `⬆️ 모델명`, downstream 체크 시 `⬇️ 모델명` 형식으로 구분 표시
- 각 체크박스에 설정된 depth 이하의 모델만 선택 항목에 포함
- counter(depth) 값 변경 시 이전 exclude 선택 자동 초기화
- Lineage 분석/초기화 버튼과 무관 — 재분석해도 선택 유지
- 모델 자체를 변경하면 초기화됨

**생성되는 dbt 명령어 형식:**
```bash
# 예시: orders_mart 기준, upstream 2단계, stg_customers 제외
dbt run --select 2+orders_mart --exclude stg_customers ...
```
...
Run 설정과 완전히 분리된 **순수 시각화** 영역.

| 컨트롤 | 설명 |
|--------|------|
| ➕ ➖ (업스트림) | 시각화에 표시할 업스트림 depth 조정 |
| 🧬 Lineage 분석 | `target/manifest.json` 파싱, 전체 depth 10까지 캐시 |
| ➕ ➖ (다운스트림) | 시각화에 표시할 다운스트림 depth 조정 |
| 🔄 Lineage 초기화 | 시각화 초기화 (exclude 선택 유지) |

**Lineage 분석 결과:**
- 업스트림: 의존 모델/소스 계층을 왼쪽에 표시
- 다운스트림: 현재 모델을 참조하는 모델 계층을 오른쪽에 표시
- 중앙: 현재 포커스 모델 (`▶ 포커스 모델` / `▶ 현재 모델` 캡션 구분)
- 소스(Source)는 🔌 아이콘과 함께 클릭 불가 버튼으로 표시
- 각 모델 버튼 클릭 → 해당 모델로 **포커스 이동** (run 설정의 `--select` 모델은 변경 안됨)
- 총 표시 컬럼 수가 많을수록 폰트 자동 축소

> ⚠️ **Lineage 시각화의 Depth와 Run의 Upstream/Downstream Depth는 별개로 동작.**
> 시각화 depth 조정이 run 명령어에 영향을 주지 않음.
...
- dbt compile/run 시 `data_interval_start / data_interval_end` vars로 전달
- 종료 < 시작이면 실행 버튼 비활성화
...
- **DBT Compile** 클릭 → `dbt compile` 실행 (Run 범위 설정 반영)
...
1. **🔍 Command Review** — 실행될 dbt run 명령어 미리 확인
   - `--select`, `--exclude`(있는 경우), `--target`, `--vars` 포함
...
*최초 작성: 2026-03-27 | 최종 업데이트: 2026-03-28*
*주요 변경: Lineage와 Run 범위 설정 분리, Exclude 멀티셀렉트 추가*
